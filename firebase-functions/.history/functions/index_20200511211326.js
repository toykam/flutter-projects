...
exports.onNewUser = functions.firestore.document('wallet/{walletId}').onUpdate(async(snapshot, context) => {
    ...
    var payload = {
        ...
        data: {
            message: `You have a new balance of ${newDatabalance}`,
    ...
    return admin.messaging().sendToDevice(tokens, payload).then((sent) => {
        console.log('Notification Sent');
    }).catch((error) => {
        console.log(`Notification not Sent. An error occurred! ${error}`);
        // return 'Uable to send notification because of error';
    });
});